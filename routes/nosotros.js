...
export function renderNosotros(container) {
  const testimoniosAprobados = JSON.parse(localStorage.getItem("comentariosAprobados") || "[]")
    .filter(c => c.tipo === 'testimonio' && c.aprobado); 
  ...
  const testimoniosItems = testimoniosAprobados.length > 0
    ? testimoniosAprobados.map((t, i) => `
      ...
      `).join('')
    : `<div class="carousel-item active"><div class="text-center px-4">
         <p class="mb-3">Aún no hay testimonios aprobados. ¡Sé el primero en dejar el tuyo!</p>
       </div></div>`;
